...
from resnet_sp import resnet50
writer = SummaryWriter(time.time())
...
if args.test:
    ...
    args.data = []
    args.prof = True
    args.data.append('/home/ubuntu/data/train/')
    args.data.append('/home/ubuntu/data/val/')
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint['state_dict'] = {k[2:]:v for k,v in checkpoint['state_dict'].items()}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        avg_train_time, train_acc1, train_acc5 = train(train_loader, model, criterion, optimizer, epoch)
        ...
        if args.local_rank == 0:
            ...
            }, is_best, filename=str(prec1)+'_'+str(epoch)+'.pth')
            if epoch == args.epochs - 1:
                print('##Top-1 {0}\n'
                      '##Top-5 {1}\n'
                      '##Perf  {2}'.format(prec1, prec5, args.total_batch_size / total_time.avg))
        writer.add_scalars('val',{'acc1':prec1,'acc5':prec5},epoch)
        writer.add_scalars('train',{'acc1':train_acc1,'acc5':train_acc5},epoch)
